...
<!--  when = else if /// oterwise = else  /// choose = 영역이라고 생각 -->
<c:choose>
	<c:when test="${dbId == inputId && dbPwd eq inputPwd }"><c:redirect url="main.jsp"/></c:when>
	<c:otherwise><c:redirect url="loginForm.jsp"/></c:otherwise>		
</c:choose>
--%>
...
<c:choose>
	<c:when test="${dbId eq inputId }">
		<c:choose>
			<c:when test="${dbId eq inputPwd }">
				로그인 성공
				<script type="text/javascript">
					alert('로그인 성공')
				</script>
			</c:when>
			<c:otherwise>비밀번호 틀림</c:otherwise>
		</c:choose>
	</c:when>
	<c:otherwise><b style="color:red;">존재하지 않은 아이디 입니다</b></c:otherwise>
</c:choose>
